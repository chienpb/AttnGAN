...
class CNN_ENCODER (nn.Module) :
    def __init__(self , nef) :
        ...
        for param in model.parameters():
            param.requires_grad = False
        print('Load pretrained model efficient net v2')

        self.define_module(model)
    def define_module(self, model):
        # To create (4,1280 , 1, 1)
        self.extract_global_feature = model.features
        self.avgpool = model.avgpool 
        # To create ([4, 256, 17, 17])
        self.extract_local_feature = model.features[:7]
        # To map to 256
        self.emb_cnn_code = nn.Linear(1280, self.nef)
    def forward(self, x):
        features = None 
        x = nn.Upsample(size=(524, 524), mode='bilinear')(x)
        # create 4 , 1280 , 1 , 1
        x_1 = self.extract_global_feature (x)
        x_1 =  self.avgpool (x_1)
        # create 4,1280
        x_1 = x_1.view(x_1.size(0), -1)
        # create 4 , 256
        cnn_code = self.emb_cnn_code (x_1)
        
        # create ([4, 256, 17, 17])
        features = self.extract_local_feature  (x)
